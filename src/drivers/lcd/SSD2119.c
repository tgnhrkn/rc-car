...
void LCD_GPIOInit(void){
    ...
    GPIO_PORTA_DATA_R |= 0xF0;            // Start 4-7 high
...
void ADC_SetChannel(unsigned char channelNum){
    ...
    //ADDED BY NATHAN
    if(channelNum == 100){
      ADC0_SSCTL3_R = (0x0E); //set END0 bit, temp sensor as 1st sample,
...
coord Touch_GetCoords1(void){
    ...
    xVals[filterCounter] = Touch_ReadX();
    yVals[filterCounter] = Touch_ReadY();
...
long Touch_GetCoordsBETTER(void){
    long result, temp, xPos, yPos;
